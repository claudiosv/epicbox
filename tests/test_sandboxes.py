...
from .utils import get_swarm_nodes, is_docker_swarm
...
def test_run_memory_limit(profile):
    result = run(
        profile.name, 'python3 -c "[1] * 10 ** 8"', limits={"cputime": 10, "memory": 8}
    )
...
def test_without_processes_limit(profile):
    ...
    assert not result["exit_code"]
    result = run(
        profile.name, "for x in {0..100}; do sleep 1 & done", limits={"processes": -1}
    )
    assert not result["exit_code"]
...
def test_run_network_enabled(profile):
    ...
    result = run(
        profile.name, "curl -I https://httpbin.org/status/200", limits={"realtime": 15}
    )
...
def test_write_files(docker_client, docker_image):
    command = (
        "/bin/bash -c " '"stat -c %a /sandbox && ls -1 /sandbox && cat /sandbox/*"'
    )
    name = "epicbox-test-" + str(uuid.uuid4())
    working_dir = config.DOCKER_WORKDIR
    container = docker_client.containers.create(
        docker_image, command=command, name=name, working_dir=working_dir
    )
    files = [
